...
class GraficoPorResposavelListView(PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
         ...
         self.queryset = Grafico.objects.filter(usuario=self.request.user)
         descricao = form.get('descricao')
         query = get_queryset_filter(self, form, descricao)
         return query
...
class GraficoMeuListView(ListView):
    # permission_required = ADD_CHAMADO
    template_name = 'grafico/grafico_list.html'
    model = Grafico
    queryset = Grafico
    ...
    def get_queryset(self):
         ...
         form = self.request.GET
         self.queryset = Grafico.objects.filter((Q(solicitante=self.request.user)| Q(responsavel=self.request.user)), excluido=False)
         descricao = form.get('descricao')
         query = get_queryset_filter(self,form,descricao)
         return query
