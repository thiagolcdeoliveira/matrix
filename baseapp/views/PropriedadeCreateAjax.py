...
from baseapp.forms.propriedade import PropriedadeAjaxForm


class PropriedadeCreateAjaxView(View):
    '''
   ...
   :URl: http://ip_servidor/pedido/cadastrar/
    ...
    def post(self, request):
        ...
        '''
        form = PropriedadeAjaxForm(request.POST, request.FILES)
        print(request.FILES)
        print(request.POST)
        if form.is_valid():
            ...
            data['campo_alterar'] = 'id_propriedade'
            data['sucesso'] = True
            return JsonResponse(data)
        else:
            print(form.errors)
